...
const Comment = () => {
  ...
  //  const totalComment = useSelector(store => store?.comments?.intialComment);
   ...
   async function getComment(pageToken="")
    {

        setLoading(true);

        // 2.fetData
      const data = await fetch(`https://youtube.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&videoId=${videoId}&maxResults=100&key=${GOOGLE_API_KEY}&pageToken=${pageToken}`);


       
        const jsonInfo = await  data.json();

        // check if next page information is available ?
        setNextPageToken(jsonInfo?.nextPageToken);

        // combine data with prev data
        setComment((prev)=>[...prev, ...jsonInfo?.items]);
